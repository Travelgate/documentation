...
The HotelX API provides paginated responses to handle cases where the number of hotels from the selected supplier exceeds 500. The default response page contains 500 hotels, but you can specify the number of objects returned per page using the `maxSize` field in the criteria variable explained before to retrieve between 1 and 500 hotels.
