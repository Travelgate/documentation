...
These operations are **Buyer-driven**: you (the Buyer) initiate the call through Travelgate, the Seller processes it and returns the data, and you receive the result. `Reservation List` and `Reservation Read` return the bookings you made against the Seller — they are **not** a way for a Seller to look up the bookings received in our platform.
:::
