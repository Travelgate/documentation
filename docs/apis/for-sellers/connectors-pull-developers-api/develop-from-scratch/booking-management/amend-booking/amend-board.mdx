...
### **Step 1: Define your AmendBookingBoard responses (Mocks)**
...
**Example of a AmendBookingBoardRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Board.Models.Request;

public class AmendBoardRequest
{
...
```csharp
internal static class AmendmentsBoardExtensions
{
    public static void AddAmendBookingBoardServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendBoardRequest, AmendBoardResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingBoardOperation<AmendBookingBoardOperation, AmendBoardRequest, AmendBoardResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingBoardOperation validators**

There are two previous validations that serve as a filter so the buildrequest and the parseresponse are as safe as possible. They can be found in the AmendBookingBoardOperation.cs class:

```csharp
internal partial class AmendBookingBoardOperation :
    IAmendBookingBoardOperation<AmendBoardRequest, AmendBoardResponse, AccessModel>
{
...
![AmendBookingBoardOperation](/img/docs/connectors-pull-developers-api/AmendBookingBoardOperation.png)
...
public bool TryValidateModelRequest(
    AmendBookingBoardConnectorRequest connectorsRequest, 
...
- Object of the requests from the models previously created (AmendBoardRequest).
...
![Build Request AmendBoard](/img/docs/connectors-pull-developers-api/BuildRequestAmendBoard.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingBoard;
...
internal partial class AmendBookingBoardOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendBoardRequest>> BuildRequests(
        AmendBookingBoardConnectorRequest connectorsRequest,
        AmendBookingBoardParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingBoardRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingBoardRq.Input.Reference.SupplierLocator;

        AmendBookingBoardConnectorRequest amendBookingBoardConnectorRequest = BuildAmendBookingBoardConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendBoardRequest> supplierRequest = new(
            amendBookingBoardConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendBookingBoardConnectorRequest BuildAmendBookingBoardConnectorRequest(
        string clientReference,
        string sellerReference) 
    {
        //The request towards the Seller system
      
        return new AmendBookingBoardConnectorRequest()
        {
...
We will be implementing the "ParseResponse" step inside AmendBookingBoardOperation:

File location: "**ConnectorsIntegration\Amend\Boards\Operations\AmendBookingBoardOperation.ParseResponse.cs**"

![Build Response AmendBookingBoard](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingBoards.png)
...
internal partial class AmendBookingBoardOperation
{
    public AmendBookingBoardConnectorResponse ParseResponses(
        AmendBookingBoardConnectorRequest connectorsRequest,
        AmendBookingBoardParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendBoardResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingBoardConnectorResponse(ParseSupplierResponse(supplierResponse));
    }
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingBoardDevelopment**" into the original repository.
