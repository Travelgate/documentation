...
ChannelX extracts the product loaded between the Buyer and the Channel Managers into our [Inventory Tool](/docs/apps/inventory/extranet/overview) and sends it to the Buyer's system. To get all the product details into their system, Buyers just need to go to the **Full Copy** tab in the Inventory's extranet. Once that's done, when the Channel Manager makes changes, ChannelX sends a smaller request **(Delta)** to let Buyers know about the updates.
...
This can be done manually in the Extranet or with one of our two newly [Inventory Set Up GraphQL API](/docs/apis/for-buyers/inventory-buyers/inventory-set-up-graphql-api/quickstart) and [Inventory Set Up CSV API](/docs/apis/for-buyers/inventory-buyers/inventory-set-up-csv-api/quickstart).  
This configuration allows the Channel Manager to map all this information and push availability and prices for the Buyer.

&nbsp;
![ChannelX Overview](https://storage.travelgate.com/docs/channel-X.svg)


:::caution

ChannelX Push Buyers API is a non-standalone API that exclusively retrieves to the Buyer's system product that have been previously pushed to the [Inventory Tool](https://inventory.xmltravelgate.com/Account/Login?ReturnUrl=%2f). To enable bookings, it is essential to also develop [HotelX Pull Buyers API](/docs/apis/for-buyers/hotel-x-pull-buyers-api/quickstart) 
...
ChannelX provides a robust XML-based API that will enable you to push all the product to your system, allowing you to create and display price calendars. Additionally, you will have full synchronization of products whenever needed, reducing network latency for availability searches.

### 1. Configure your system to operate with ChannelX

#### API Endpoint

For the ChannelX API, it is essential that you [send your endpoint to Travelgate](./making-requests/endpoint).
...
In the case of the ChannelX API, Travelgate will send requests to the Buyer's system, and the Buyer will respond with either success or error.

**Requests and Responses**

ChannelX has three main methods. You will have to configure your system to accept the following requests: 
...
### 3. ChannelX pushes the information to your system
...
To enable bookings, it is **essential** to develop [HotelX Pull Buyers API](/docs/apis/for-buyers/hotel-x-pull-buyers-api/quickstart.mdx).
