...
# Inventory-Push GraphQL API

Inventory-Push is a travel API based on GraphQL. Unlike XML Soap APIs, GraphQL allows to request and receive only the necessary data. Our Gateway, written in NodeJS, is the sole endpoint for all our APIs.
...
Queries are “read only” and are used to get data from our system. For example, getting a list of Channel Managers' hotel vacancies created in our Inventory is a `Query`.
