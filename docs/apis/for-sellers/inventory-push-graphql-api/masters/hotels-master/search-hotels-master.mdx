...
Now, we want to filter previous results by introducing the filter `hotelName` and we are trying to search for hotels that are close to the sea, so we will assume that some hotels have "mar" in their name.
