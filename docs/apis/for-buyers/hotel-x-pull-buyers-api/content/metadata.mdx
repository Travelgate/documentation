...
* `allowsCurrency`
* `release` (Minimum days required in between booking date and checking date, if the value is zero then there is no limitation. If the value is -1, buyers are allowed to make reservations for the day before the check-in. Useful in edge cases where the supplier's timezone is ahead.)
