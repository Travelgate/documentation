...
Instead of validating hotel or board mappings, this section lets you simulate the room standardization process and preview the normalized output that Buyers will receive in API responses.
...
* See the standardized FastX room description generated by the engine.
...
5. Review the output card:
    * **Standardized room description** (FastX standardized text).
    * **Generated room code**.
...
<details>
    ...
    <div>
         <div>
          Buyers with FastX access performing a **Content request** in the Supplier context will receive FastX codes for Hotels and Boards within the response's mapping node. Room codes cannot be obtained this way, as they are standardized through the Search response.  
