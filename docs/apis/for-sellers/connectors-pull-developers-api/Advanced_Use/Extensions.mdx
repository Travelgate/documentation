...
```csharp
AddSerializerDependencySearch<SearchRequest, ResponseWrapper<List<SearchResponse>>>(services); 
...
```csharp
AddMultiSerializer<SearchRequest, ResponseWrapper<List<SearchResponse>>>(services); 
services.AddSearchOperation<SearchOperation, SearchRequest, ResponseWrapper<List<SearchResponse>>, AccessModel>(TgxPlatform.Name, configuration);
