...
1. However, it's worth mentioning that certain integrations developed at TravelgateX (not all of them) require **multiple transactions per method to accommodate the Seller's API workflow**. For example, our Quote method may need 2 or 3 calls to the Seller's system to verify availability, rates, etc.
