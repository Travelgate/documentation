...
The Mapping plugin allows Buyers to perform the entire booking flow using their own hotel, board, room, and rate codes instead of the standard FastX codes or the supplier codes.

:::info Context Mode
Use Mapping only when your integration needs Buyer-owned codes. For mode selection criteria, see [Context Decision Matrix](/kb/connectivity-products/for-buyers/hotel-x/hotel-x-credentials#context).
:::

To make this possible, the Buyer uploads mapping files that define the relationship between their own codes and those of the connected Suppliers.
Once these mappings are in place, HotelX automatically handles the conversion during the booking process:

When you send a Search or Quote request using your Buyer context, the system translates your Buyer codes into the corresponding Supplier codes before sending the request.

In the response, you’ll receive both:

* The supplier information.

* Your mapped Buyer codes for the entities you’ve configured (e.g., hotels, boards, rooms, rates, etc.)

The Mapping plugin supports the following mapping categories:
Hotel Map, Board Map, Room Map, Rate Map, Amenity Map, Promotion Map, and Room Description Map.
...
If your search query's context is defined as your own Buyer context (meaning it differs from the supplier's context), HotelX will make an effort to map all entities (such as hotels, boards, etc., excluding amenities and promotions) to align with the query's context.
...
This approach lets HotelX exclusively focus on mapping the entities you've specified in your query.
