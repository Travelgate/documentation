...
Once the **MockServer** is configured ([see MockServer Documentation](MockServer_Documentation.mdx)), you can execute the integration tests. These tests validate the Connector's functionality and ensure that the Metadata ([see Metadata Documentation](../Advanced_Configuration/Metadata_Definition.mdx)) aligns with the integration.
...
2. **Configure `appsettings.json`:**
   ...
   ```json
   {
     "Supplier": "SUPPLIERCODE_DEFINED_BY_TGX",
     "ConnectorsPlatform": "PLATFORMCODE_DEFINED_BY_TGX"
   }
   ```
