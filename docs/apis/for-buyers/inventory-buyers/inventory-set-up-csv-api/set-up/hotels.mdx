...
The hotels you reference here must already exist in the **Hotels Master** catalog (either created previously through the `hotelsMaster` file or shared with your client). The combination **`Id` + `Context`** is used to look up the hotel — if either value does not match the master, processing fails with `Hotel not found`.

For Channel Managers using native codes, the `Context` of this file **must be identical** to the `Context` declared in the corresponding `hotelsMaster` row.
:::

:::caution Avoid empty rows
A row containing only semicolons (e.g. `;;;;;;;`) is **not** ignored automatically and will be sent to the provider as an invalid hotel. Make sure to delete blank rows from the CSV before uploading.
:::
