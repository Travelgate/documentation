...
There are certain elements that need to be included in every request, namely common elements. These common elements include `timeoutMilliseconds`, `languageCode`, `registerTransactions`, and the supplier's `Configuration` etc.
...
| Configuration                              | 1    |         | The info required to access the supplier’s system. You will be able to see this information when connecting with the seller. Afterward, you will find the values to add in Configuration in the [RuntimeConfiguration](../content/runtime-configuration#runtimeconfiguration-response) content call.                         |
