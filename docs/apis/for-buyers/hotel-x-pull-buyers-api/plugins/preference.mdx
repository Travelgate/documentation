...
4. Take the first '*n*' options (4 by default, configurable with the **`optionsPerKey`** parameter). That is, this will involve selecting the n cheapest options once the prices have been recalculated, taking the preference rules into account. There is one exception
