...
* **Base Currency:**
   ...
   * TG Distribution system operates using EUR as the base currency for all the internal operations, so if the file does not uses EUR as the base currency then the TG Distribution team must be noticed of this point and needs to know which is the base currency.
