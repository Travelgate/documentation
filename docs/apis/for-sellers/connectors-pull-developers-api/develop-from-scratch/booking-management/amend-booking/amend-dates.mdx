...
### **Step 1: Define your AmendBookingDates responses (Mocks)**
...
**Example of a AmendBookingDatesRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Dates.Models.Request;

public class AmendDatesRequest
{
...
```csharp
internal static class AmendmentsDatesExtensions
{
    public static void AddAmendBookingDatesServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendDatesRequest, AmendDatesResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingDatesOperation<AmendBookingDatesOperation, AmendDatesRequest, AmendDatesResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingDatesOperation validators**

There are two previous validations that serve as a filter so the buildrequest and the parseresponse are as safe as possible. They can be found in the AmendBookingDatesOperation.cs class:

```csharp
internal partial class AmendBookingDatesOperation :
    IAmendBookingDatesOperation<AmendDatesRequest, AmendDatesResponse, AccessModel>
{
...
![AmendBookingDatesOperation](/img/docs/connectors-pull-developers-api/AmendBookingDatesOperation.png)
...
public bool TryValidateModelRequest(
    AmendBookingDatesConnectorRequest connectorsRequest, 
...
- Object of the requests from the models previously created (AmendDatesRequest).
...
![Build Request AmendDates](/img/docs/connectors-pull-developers-api/BuildRequestAmendDates.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingDates;
...
internal partial class AmendBookingDatesOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendDatesRequest>> BuildRequests(
        AmendBookingDatesConnectorRequest connectorsRequest,
        AmendBookingDatesParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingDatesRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingDatesRq.Input.Reference.SupplierLocator;

        AmendBookingDatesConnectorRequest amendBookingDatesConnectorRequest = BuildAmendBookingDatesConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendDatesRequest> supplierRequest = new(
            amendBookingDatesConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendBookingDatesConnectorRequest BuildAmendBookingDatesConnectorRequest(
        string clientReference,
        string sellerReference) 
    {
        //The request towards the Seller system
      
        return new AmendBookingDatesConnectorRequest()
        {
...
We will be implementing the "ParseResponse" step inside AmendBookingDatesOperation:

File location: "**ConnectorsIntegration\Amend\Dates\Operations\AmendBookingDatesOperation.ParseResponse.cs**"

![Build Response AmendBookingDates](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingDates.png)
...
internal partial class AmendBookingDatesOperation
{
    public AmendBookingDatesConnectorResponse ParseResponses(
        AmendBookingDatesConnectorRequest connectorsRequest,
        AmendBookingDatesParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendDatesResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingDatesConnectorResponse(ParseSupplierResponse(supplierResponse));
    }
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingDatesDevelopment**" into the original repository.
