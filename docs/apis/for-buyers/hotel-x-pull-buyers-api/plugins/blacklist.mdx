...
1. anage your Blacklist rules (via the website or by uploading a file via SFTP).
...
#### Evaluation
- If a Search request is executed using access `ACCESS_ABC` → only the selected hotels are queried.
