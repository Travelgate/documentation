...
- If a Buyer is transitioning between Legacy and HotelX, traffic may appear on both APIs. Before applying `access_in`, retrieve the Buyer Connection Type (`buyerIntegrationType`) with the [Get Profiles query](/docs/apis/for-buyers-and-sellers/social-api/get-profiles) or confirm it in [My Connections](/kb/platform/app-features/connections/my-connections/managing-connections/connections-details#additional-access-information).
...
|---|---|---|---|
| `buyer` (scalar `String`) | `buyer_in` (array of `String`) | Buyer Organization Code | Query your Buyers list in the [Admin API](https://docs.travelgate.com/docs/apis/for-buyers-and-sellers/admin-api/overview) to retrieve the `orgCode` |
| `seller` (scalar `String`) | `seller_in` (array of `String`) | Seller Organization Code | Query your Sellers list in the [Admin API](https://docs.travelgate.com/docs/apis/for-buyers-and-sellers/admin-api/overview) to retrieve the `orgCode` |
