...
| **FUN-02** | | | Business logic validation | Validate the underlying business logic for consistency and accuracy. Minimum shopping flow (search, quote?, book) | Mandatory | |
...
| **CNP-01A** | | | Deadline meaning | Confirm that the deadline (if there is an attribute with this name) corresponds to the **start** of the policy. | Mandatory | |
...
| **AVAI-08**| | | Only requires one HTTP call. | Mandatory | |
...
| **CNX-02** | | | Amount to be paid (cancel penalty) | | Mandatory | |
| **CHB-01** | CheckBookingsByReference | Highly recommended | Reservation status | | Mandatory | |
| **CHB-02** | | | Updated cancellation price | Verify that when a booking is cancelled, the API returns the **updated cancellation price**. | Mandatory | |
| **CHBD-01** | CheckBookingsByDate | Recommended | Reservation status | | Mandatory | |
| **CHBD-02** | | | Updated cancellation price | Verify that when a booking is cancelled, the API returns the **updated cancellation price**. | Mandatory | |
