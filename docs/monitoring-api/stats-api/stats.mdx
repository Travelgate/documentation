...
| `OPERATION_TYPE` | Group by operation type (`SEARCH`, `QUOTE`, `BOOKING`, `CANCEL`, `SCHEDULER`, `CREATE`, `READ`, `UPDATE`, `DELETE`, `EXECUTE`, `OTHER`) |
...
:::note
This is the complete list of `StatsDataGroupBy` enum values. You can combine multiple values in a single `groupBy.data` array (for example, `["OPERATION_TYPE", "ERROR_CODE"]`) to create multi-dimensional aggregations.
:::
...
The `operationType_in` filter uses the `OperationType` enum:
...
| `SCHEDULER` | Scheduler-related operations |
