...
* **Header Row:**
    ...
    * **Countries:** List of countries allowed for this rule
    * **NoCountries:** List of countries not allowed for this rule
...
| `currency`       | ISO 4217 code | If you want to override the reservation currency and specify an exact currency, use this parameter. If used, amount must also be provided.                               | Reservation currency of the booking                             | NO        |
...
In some cases, an error may occur during the booking process of HotelX. This can result in a virtual credit card (VCC) being created even though the hotel reservation was not successfully completed.

To handle this scenario, clients can use the provided GraphQL API to cancel the VCC on their own.


### Cancel 
The cancel operation allows you to cancel virtual credit card made during booking process of HotelX. The returned fields include:
...
Settings are the common configurations used to construct requests to the supplier/s. By default, we apply the same configuration to all HotelX clients.
...
List of possible value: [Here](#listcardtype)

### Examples

#### Cancel with the `id` from the book response of HotelX
