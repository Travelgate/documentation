...
Claro, aquí tienes la tabla actualizada con las descripciones revisadas para los parámetros `useGross` y `amount`:
...
| `useGross`       | true or false | Indicates that the amount to load onto the VCC will be the reservation value. If true, the gross value will be used; if false, the net value will be used. Not compatible with the 'amount' parameter. | Net option’s value                                              | NO        |
| `amount`         | two decimals  | If you do not want to load the VCC with the reservation value and prefer to specify an exact amount, use this parameter. Not compatible with the 'useGross' parameter. If not provided, 'useGross' will be used based on the reservation price; if neither is indicated, the net reservation value will be used.          | If not set, use 'useGross' based on reservation price; if neither, use net reservation value | NO        |
