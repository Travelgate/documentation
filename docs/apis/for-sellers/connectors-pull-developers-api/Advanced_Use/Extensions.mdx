...
In Connectors you may need polling for some sellers, this means a pagination is needed. In this case, a **AddSearchPollingOperationFor** extension can be used:
...
internal static class SearchExtensions
{
        ...
        IConfiguration configuration)
    {
        ...
        services.AddSearchOperation<SearchOperation, SearchRequest, SearchResponse, AccessModel>(TgxPlatform.Name,
            configuration).AddSearchPollingOperationFor<SearchRequest, SearchResponse, EhtAccessModel>();
    }
}
```

This will allow the operation to work with pagination, afterwards just use it your Search ParseResponse:
```csharp
public SearchConnectorResponse ParseResponses(
    SearchConnectorRequest connectorsRequest,
    SearchParameters<EhtAccessModel> connectorParameters,
