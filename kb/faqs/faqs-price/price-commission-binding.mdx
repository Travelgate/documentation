...
## Understanding Commission in HotelX and Legacy APIs

In HotelX, both net and gross prices are provided for easy calculation. In the Legacy Pull Buyers API, only `amount`, `commission`, and `binding` information is available. Example:
```xml
...
Since the HotelX API does not include a commission field, you can calculate it using:
```math
