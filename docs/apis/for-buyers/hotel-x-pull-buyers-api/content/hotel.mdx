...
#### Search complete hotel list from a supplier
...
For example, if we want to obtain only hotels from "Santorini", we can use the destination code for that zone. In our case, our test suppliers as code "101402" as Santorini, so we need to specify this code in the `destinationCodes` field:

<GraphqlSample query={hotelsQuery} variables={hotelsByDestinationCodeVariables} />

#### Search for specific hotel code
...
#### Search for a specific creation date
