...
### **Step 1: Define your AmendBookingRemarks responses (Mocks)**
...
**Example of a AmendBookingRemarksRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Remarks.Models.Request;

public class AmendRemarksRequest
{
...
```csharp
internal static class AmendmentsRemarksExtensions
{
    public static void AddAmendBookingRemarksServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendRemarksRequest, AmendRemarksResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingRemarksOperation<AmendBookingRemarksOperation, AmendRemarksRequest, AmendRemarksResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingRemarksOperation validators**

There are two previous validations that serve as a filter so the buildrequest and the parseresponse are as safe as possible. They can be found in the AmendBookingRemarksOperation.cs class:

```csharp
internal partial class AmendBookingRemarksOperation :
    IAmendBookingRemarksOperation<AmendRemarksRequest, AmendRemarksResponse, AccessModel>
{
...
![AmendBookingRemarksOperation](/img/docs/connectors-pull-developers-api/AmendBookingRemarksOperation.png)
...
public bool TryValidateModelRequest(
    AmendBookingRemarksConnectorRequest connectorsRequest, 
...
- Object of the requests from the models previously created (AmendRemarksRequest).
...
![Build Request AmendRemarks](/img/docs/connectors-pull-developers-api/BuildRequestAmendRemarks.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingRemarks;
...
internal partial class AmendBookingRemarksOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendRemarksRequest>> BuildRequests(
        AmendBookingRemarksConnectorRequest connectorsRequest,
        AmendBookingRemarksParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingRemarksRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingRemarksRq.Input.Reference.SupplierLocator;

        AmendBookingRemarksConnectorRequest amendBookingRemarksConnectorRequest = BuildAmendBookingRemarksConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendRemarksRequest> supplierRequest = new(
            amendBookingRemarksConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendBookingRemarksConnectorRequest BuildAmendBookingRemarksConnectorRequest(
        string clientReference,
        string sellerReference) 
    {
        //The request towards the Seller system
      
        return new AmendBookingRemarksConnectorRequest()
        {
...
We will be implementing the "ParseResponse" step inside AmendBookingRemarksOperation:

File location: "**ConnectorsIntegration\Amends\Remarks\Operations\AmendBookingRemarksOperation.ParseResponse.cs**"

![Build Response AmendBookingRemarks](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingRemarks.png)
...
internal partial class AmendBookingRemarksOperation
{
    public AmendBookingRemarksConnectorResponse ParseResponses(
        AmendBookingRemarksConnectorRequest connectorsRequest,
        AmendBookingRemarksParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendRemarksResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingRemarksConnectorResponse(ParseSupplierResponse(supplierResponse));
    }
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingRemarksDevelopment**" into the original repository.
