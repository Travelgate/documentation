...
* Consider that the `deltaPrice` sets the price tolerance between Quote and Book. For instance, if the Quote stage displays a price of 100€ and the `deltaPrice.amount` is 5, a price up to 105€ will still secure a confirmed booking. Similarly, if `deltaPrice.percent` is 5 (meaning 5%), a price up to 105€ will be accepted.
