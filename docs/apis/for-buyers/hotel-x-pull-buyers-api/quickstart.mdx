...
The Search query is where the adventure begins in our shopping journey! In this example, we'll explore availability for a **single hotel using its FastX code**. This hotel is provided by  two different test Suppliers, so we'll make the request including both supplier accesses: `'2'` and `'5647'`. 
