...
[`OptionRefID`](/kb/connectivity-products/for-buyers/hotel-x/booking-flow/option-identifiers) is a unique identifier assigned to each option. **It must not be modified** and **must not be used or interpreted externally**, as it is an internal Treavelgate system identifier.
