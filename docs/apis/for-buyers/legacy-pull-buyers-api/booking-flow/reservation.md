...
| ReservationRQ/OnRequest     				| 1  		| Boolean	| Indicates if you want to receive the onrequest options in AvailRS, as long as the supplier returns it in this method (see [MetaData](../content/meta-data/) in order to verify if a supplier implements it).	|
...
| ReservationRQ/Nationality   				| 1		| String	| Nationality of the Holder (use ISO3166_1_alfa_2 , see [MetaData](../content/meta-data/) in order to verify if a supplier implements it).  |
