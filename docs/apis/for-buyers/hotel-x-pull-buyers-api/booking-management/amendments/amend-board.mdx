...
import {boardAmendmentQuoteQuery, boardAmendmentQuoteVariables} from "../../../../../../src/graphql/hotel-x/board.amendment.quote.query";
import {boardAmendmentCommitMutation, boardAmendmentCommitVariables} from "../../../../../../src/graphql/hotel-x/board.amendment.commit.mutation";
...
Additionally you'll need to indicate the new board code you want for your reservation with the `board` input. For example:
...
#### Board amendment quote with the `bookingID` from the book response

<GraphqlSample query={boardAmendmentQuoteQuery} variables={boardAmendmentQuoteVariables} />
...
* `clientTokens` (Used to identify the origin of the request, this is only used in plugins)
* `businessRules`
* `operationTimeout` (Timeout in milliseconds for HotelX connection. Will close Buyer connection if exceeded. Must be higher than `timeout`.)


### Requests Examples

<GraphqlSample query={boardAmendmentCommitMutation} variables={boardAmendmentCommitVariables} />