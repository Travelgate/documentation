...
:::note
This operation simulates what would happen if you execute the `AmendBookingDates` operation, without actually committing the changes.
:::

## **Mock**

### **Step 1: Define your AmendBookingDates Simulate responses (Mocks)**
...
File location: "**test\MockServer\Tests\AmendBookingDates\Two_Adults_Two_Days.txt**".

![Mock Example](/img/docs/connectors-pull-developers-api/MockTwoAdultsAmendDatesSimulate.png)
...
![Response example](/img/docs/connectors-pull-developers-api/TemplateStructureModelsAmendBookingDates.png)

**Example of a AmendDatesSimulateRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Dates.Models.Request;

public class AmendDatesSimulateRequest
{
    public NewDate NewDates { get; set; }
...
```
File location: "**ConnectorsIntegration\Amends\Dates\Models\Request\AmendDatesRequest.cs**".

**Example of a AmendDatesResponse model**:

```csharp
namespace ConnectorsIntegration.Amends.Dates.Models.Response;

public class AmendDatesResponse
{
...
```
File location: "**ConnectorsIntegration\Amends\Dates\Models\Response\AmendDatesResponse.cs**".
...
File location: "**ConnectorsIntegration\Amends\Dates\AmendmentsDatesExtensions.cs**".

If the Seller works with **JSON** format, we can specify the integration to work with **JSON** with the following:

```csharp
internal static class AmendmentsDatesSimulateExtensions
{
    public static void AddAmendBookingDatesSimulateServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendDatesSimulateRequest, AmendDatesResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingDatesSimulateOperation<AmendBookingDatesSimulateOperation, AmendDatesSimulateRequest, AmendDatesResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingDatesSimulateOperation validators**
...
```csharp
internal partial class AmendBookingDatesSimulateOperation :
    IAmendBookingDatesSimulateOperation<AmendDatesSimulateRequest, AmendDatesResponse, AccessModel>
{
    // Implement any services needed just like in SearchOperation.cs
}
```

File location: "**ConnectorsIntegration\Amends\Dates\Operations\AmendBookingDatesOperation.cs**".

![AmendBookingDatesOperation](/img/docs/connectors-pull-developers-api/AmendBookingDatesOperationSimulate.png)
...
public bool TryValidateModelRequest(
    AmendBookingDatesSimulateConnectorRequest connectorsRequest, 
    AmendBookingDatesParameters<AccessModel> connectorParameters,
...
public bool TryValidateSupplierResponses(
    AmendBookingDatesParameters<AccessModel> connectorParameters,
    IEnumerable<SupplierResponseWrapper<AmendDatesResponse>> supplierResponses,
    out IEnumerable<AdviseMessage> adviseMessages)
{
    var supplierResponseWrappers = supplierResponses as SupplierResponseWrapper<AmendDatesResponse>[] ?? supplierResponses.ToArray();
...
- Object of the requests from the models previously created (AmendDatesSimulateRequest).
- The request that the Buyer sends (connectorsRequest).
- Parameters (connectorParameters) which will have some helpers.

File location: "**ConnectorsIntegration\Amends\Dates\Operations\AmendBookingDatesOperation.BuildRequest.cs**"

![Build Request AmendDates](/img/docs/connectors-pull-developers-api/BuildRequestAmendDatesSimulate.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingDatesSimulate;
using ConnectorsIntegration.Amends.Dates.Models.Request;

namespace ConnectorsIntegration.Amends.Dates.Operations;

internal partial class AmendBookingDatesSimulateOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendDatesSimulateRequest>> BuildRequests(
        AmendBookingDatesSimulateConnectorRequest connectorsRequest,
        AmendBookingDatesParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingDatesSimulateRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingDatesSimulateRq.Input.Reference.SupplierLocator;

        AmendDatesSimulateRequest amendBookingDatesSimulateConnectorRequest = BuildAmendBookingDatesSimulateConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendDatesSimulateRequest> supplierRequest = new(
            amendBookingDatesSimulateConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendDatesSimulateRequest BuildAmendBookingDatesSimulateConnectorRequest(
        string clientReference,
        string sellerReference) 
    {
        //The request towards the Seller system
      
        return new AmendDatesSimulateRequest()
        {
            ClientReference = clientReference,
            SellerReference = sellerReference,
            NewDates = new NewDate()
            {
                CheckIn = "2026-02-17",
                CheckOut = "2026-02-18"
            }
...
:::note
The ParseResponse implementation is shared between Simulate and Commit operations, as they return the same response structure (AmendDatesResponse).
:::

We will be implementing the "ParseResponse" step inside the Simulate operation:

File location: "**ConnectorsIntegration\Amend\Dates\Operations\AmendBookingDatesOperation.ParseResponse.cs**"

![Build Response AmendBookingDates](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingDatesSimulate.png)
...
internal partial class AmendBookingDatesSimulateOperation
{
    public AmendBookingDatesSimulateConnectorResponse ParseResponses(
        AmendBookingDatesSimulateConnectorRequest connectorsRequest,
        AmendBookingDatesParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendDatesResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingDatesSimulateConnectorResponse(ParseSupplierResponse(supplierResponse));
    }

    private AmendRs ParseSupplierResponse(AmendDatesResponse supplierResponse)
    {
        ...
        BookDetails bookDetails = new BookDetails(status, new Reference(clientReference, supplierReference), price)
        {
            ...
            CheckIn = supplierOption.CheckIn,
            CheckOut = supplierOption.CheckOut
        };
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingDatesSimulateDevelopment**" into the original repository.
