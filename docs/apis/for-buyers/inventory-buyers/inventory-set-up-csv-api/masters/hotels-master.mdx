...
This document outlines the structure and required fields for two types of master hotel records represented in CSV files: (1) new hotels native to the Channel Manager and (2) new hotels with a Travelgate inventory code.
...
| Latitude    |   1   | Latitude of the hotel's geographical position in WGS84 decimal format (single decimal separator: dot or comma). Example: `36.500832` or `36,500832`. |
| Longitude   |   1   | Longitude of the hotel's geographical position in WGS84 decimal format (single decimal separator: dot or comma). Example: `-4.881620` or `-4,881620`. |
| Category    |   1   | Classification or category of the hotel. [Check the list of hotel categories](#list-of-hotel-categories) |

:::caution Common coordinate mistake
Do **not** combine the thousands separator with the decimal separator inside a coordinate (for example `36.500,832` is invalid because the dot is treated as decimal separator and the rest is rejected). Use a **single** decimal separator only.
:::

:::caution Locality must match the API catalog
The `Locality` value is matched **case-sensitively** against the catalog returned by the provider for your `client`/`supplier`. Use the exact spelling (for example `Marbella`, not `MARBELLA` or `marbella ` with trailing spaces). If you are unsure of the right spelling, query the catalog with the `localities` GraphQL endpoint or use the dropdown in the [Google Sheets CSV Builder](../google-sheets-csv-builder).
:::
...
#### CSV Format Example for Channel Managers without context
