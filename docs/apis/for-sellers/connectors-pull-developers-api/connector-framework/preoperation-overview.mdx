...
Preoperations are preparatory steps required by the supplier before executing a main operation (see [Operation Overview](operation-overview.mdx)).

These preoperations follow the same structure as regular operations and implement the `IPre<Operation>Operations<SupplierRequest, SupplierResponse, Binder>` interface. Preoperations can be applied to `Search`, `Quote`, `Book`, `CheckBookingsByReference`, `CheckBookingsByDates`, `Cancel`, and all types of `Amendments`.
