...
Please note that you can only select a maximum range of 7 days between the "from" date and the "to" date.
...
In order to obtain more detailed information of a specific booking you can use the `Booking read` panel on the left and filter by one of the booking locators (Client, Provider or TG).
