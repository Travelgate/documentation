...
Even if not all criteria are marked as mandatory, you must either use the `bookingID` from the book response or provide the set of all other criteria, including the `accessCode`, `hotel`, and `reference`, to cancel the reservation.
...
To specify your cancel settings you need to use the input `HotelSettingsInput` in your mutation variables:
