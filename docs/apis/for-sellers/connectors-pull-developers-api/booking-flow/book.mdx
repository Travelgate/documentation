...
- **Booking Status**:
  ...
  - `Cancelled`: The reservation was completed but due to a supplier error or a timeout the system will immediately cancel the reservation to prevent further possible errors.
  - `OnRequest`: Please note that a Book status may change over time: you may receive an OnRequest status in Book response, and after running a Booking Query some seconds later the status may have already changed to Ok.
