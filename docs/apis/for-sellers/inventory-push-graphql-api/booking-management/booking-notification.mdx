...
During the booking lifecycle, we'll send you a `BookNotif` request message to share reservation details. This same object is used for both booking notifications and cancellation notifications. You just need to check and let us know the reservation status. After that, Travelgate will handle the response from your system, moving forward with the reservation and updating you on the status.

The type of notification is determined by `bookDetails.status`:

- `Confirmed`: booking notification (reservation confirmation).
- `Cancelled`: cancellation notification (reservation cancellation).

No other `status` values are expected in `BookNotif`.
    ...
    <details style={{ pointerEvents: "none" }}>
      <summary>
        **status**&nbsp;<span class="required"> *</span>&nbsp; (*Enum of String*)    
        Status used to identify the notification type. Possible values:    
...
## Returned Fields
     ...
     <details>
  <summary>
    **bookNotifRs**&nbsp;<span class="required"> *</span>&nbsp; (*Object*)  
    Root object containing the response information.
  </summary>
  <details style={{ pointerEvents: "none" }}>
    <summary>
      **dateTime**&nbsp;<span class="required"> *</span>&nbsp; (*DateTime*)  
  ...
  <details style={{ pointerEvents: "none" }}>
    <summary>
      **status**&nbsp;<span class="required"> *</span>&nbsp; (*Enum of String*)  
      Status of the processed notification. Mandatory if the response is successful. Possible values:  
      `Confirmed`: when `bookNotif.bookDetails.status` is `Confirmed` (booking notification).  
      `Cancelled`: when `bookNotif.bookDetails.status` is `Cancelled` (cancellation notification).  
    </summary>
  </details>
  ...
  <details>
    ...
    <details style={{ pointerEvents: "none" }}>
      <summary>
        **code**&nbsp;<span class="required"> *</span>&nbsp; (*Integer*)  
