...
### **Step 1: Define your AmendBookingRooms Simulate responses (Mocks)**
...
File location: "**test\MockServer\Tests\AmendBookingRooms\Two_Adults_Two_Days.txt**".

![Mock Example](/img/docs/connectors-pull-developers-api/MockTwoAdultsAmendRoomsSimulate.png)
...
![Response example](/img/docs/connectors-pull-developers-api/TemplateStructureModelsAmendBookingRooms.png)

**Example of a AmendRoomsSimulateRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Rooms.Models.Request;

public class AmendRoomsSimulateRequest
{
    public List<AmendRoom> NewRooms { get; set; }
...
```
File location: "**ConnectorsIntegration\Amends\Rooms\Models\Request\AmendRoomsRequest.cs**".

**Example of a AmendRoomsResponse model**:

```csharp
namespace ConnectorsIntegration.Amends.Rooms.Models.Response;

public class AmendRoomsResponse
{
...
```
File location: "**ConnectorsIntegration\Amends\Rooms\Models\Response\AmendRoomsResponse.cs**".
...
File location: "**ConnectorsIntegration\Amends\Rooms\AmendmentsRoomsExtensions.cs**".

If the Seller works with **JSON** format, we can specify the integration to work with **JSON** with the following:

```csharp
internal static class AmendmentsRoomsSimulateExtensions
{
    public static void AddAmendBookingRoomsSimulateServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendRoomsSimulateRequest, AmendRoomsResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingRoomsSimulateOperation<AmendBookingRoomsSimulateOperation, AmendRoomsSimulateRequest, AmendRoomsResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingRoomsSimulateOperation validators**
...
```csharp
internal partial class AmendBookingRoomsSimulateOperation :
    IAmendBookingRoomsSimulateOperation<AmendRoomsSimulateRequest, AmendRoomsResponse, AccessModel>
{
    // Implement any services needed just like in SearchOperation.cs
}
```

File location: "**ConnectorsIntegration\Amends\Rooms\Operations\AmendBookingRoomsOperation.cs**".

![AmendBookingRoomsOperation](/img/docs/connectors-pull-developers-api/AmendBookingRoomsOperationSimulate.png)
...
public bool TryValidateModelRequest(
    AmendBookingRoomsSimulateConnectorRequest connectorsRequest, 
    AmendBookingRoomsParameters<AccessModel> connectorParameters,
...
public bool TryValidateSupplierResponses(
    AmendBookingRoomsParameters<AccessModel> connectorParameters,
    IEnumerable<SupplierResponseWrapper<AmendRoomsResponse>> supplierResponses,
    out IEnumerable<AdviseMessage> adviseMessages)
{
    var supplierResponseWrappers = supplierResponses as SupplierResponseWrapper<AmendRoomsResponse>[] ?? supplierResponses.ToArray();
...
- Object of the requests from the models previously created (AmendRoomsSimulateRequest).
- The request that the Buyer sends (connectorsRequest).
- Parameters (connectorParameters) which will have some helpers.

File location: "**ConnectorsIntegration\Amends\Rooms\Operations\AmendBookingRoomsSimulateOperation.BuildRequest.cs**"

![Build Request AmendRooms](/img/docs/connectors-pull-developers-api/BuildRequestAmendRoomsSimulate.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingRoomsSimulate;
using ConnectorsIntegration.Amends.Rooms.Models.Request;

namespace ConnectorsIntegration.Amends.Rooms.Operations;

internal partial class AmendBookingRoomsSimulateOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendRoomsSimulateRequest>> BuildRequests(
        AmendBookingRoomsSimulateConnectorRequest connectorsRequest,
        AmendBookingRoomsParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingRoomsSimulateRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingRoomsSimulateRq.Input.Reference.SupplierLocator;

        AmendRoomsSimulateRequest amendBookingRoomsSimulateConnectorRequest = BuildAmendBookingRoomsSimulateConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendRoomsSimulateRequest> supplierRequest = new(
            amendBookingRoomsSimulateConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendRoomsSimulateRequest BuildAmendBookingRoomsSimulateConnectorRequest(
        string clientReference,
        string supplierReference) 
    {
        //The request towards the Seller system
      
        return new AmendRoomsSimulateRequest()
        {
            ClientReference = clientReference,
            SupplierReference = supplierReference,
            NewRooms = new List<AmendRoom>()
            {
                new AmendRoom
                {
                    OccupancyRefId = 1,
...
:::note
The ParseResponse implementation is shared between Simulate and Commit operations, as they return the same response structure (AmendRoomsResponse).
:::

We will be implementing the "ParseResponse" step inside the Simulate operation:

File location: "**ConnectorsIntegration\Amends\Rooms\Operations\AmendBookingRoomsSimulateOperation.ParseResponse.cs**"

![Build Response AmendBookingRooms](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingRoomsSimulate.png)
...
internal partial class AmendBookingRoomsSimulateOperation
{
    public AmendBookingRoomsSimulateConnectorResponse ParseResponses(
        AmendBookingRoomsSimulateConnectorRequest connectorsRequest,
        AmendBookingRoomsParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendRoomsResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingRoomsSimulateConnectorResponse(ParseSupplierResponse(supplierResponse));
    }

    private AmendRs ParseSupplierResponse(AmendRoomsResponse supplierResponse)
    {
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingRoomsSimulateDevelopment**" into the original repository.
