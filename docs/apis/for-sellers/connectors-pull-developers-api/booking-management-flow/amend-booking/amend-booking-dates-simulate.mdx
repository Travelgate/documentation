---
title: "AmendBookingDatesSimulate"
sidebar_position: 3
---

## **Overview**

The `AmendBookingDatesSimulate` operation allows clients to modify the check-in and/or check-out dates of an existing booking. This operation uses the client and supplier locators to identify the reservation and processes the date change request while recalculating pricing based on the new stay period.

:::note
This operation simulates what would happen if you execute the [`AmendBookingDates`](amend-booking-dates.mdx) operation, without actually committing the changes.
:::

The AmendBookingDatesSimulate operation ensures:
...
```csharp
var amendBookingDatesSimulateRq = new AmendBookingDatesSimulateRq
{
    Input = new AmendDatesSimulateInput
    {
...
<!-- TABLE BEGIN Connectors.Pull.Hotel.Domain.Contracts.Amendments.AmendBookingDates.Request.AmendBookingDatesSimulateRq -->
| Name | Relation | Type | Description |
|------|:--------:|:----:|-------------|
| **Input** | 1 | [AmendDatesSimulateInput](/docs/apis/for-sellers/connectors-pull-developers-api/api-reference/amenddatessimulateinput) | Gets or initializes the input details for simulating the dates amendment, including specific information about the booking and the new dates. |
...
| **Input/AmendDates** | 1 | [AmendDates](/docs/apis/for-sellers/connectors-pull-developers-api/api-reference/amenddates) | Gets the amended dates for the booking. |
...
The `AmendBookingDatesSimulate` operation provides:
- **Amendment Status**:
...
var amendRs = new AmendRs
{
    ...
    BookDetails = new List<BookDetails>
    {
        new BookDetails
        {
            ...
            Price = Price.BuildNetPrice(Currency.EUR, 250.00, 300.00),
