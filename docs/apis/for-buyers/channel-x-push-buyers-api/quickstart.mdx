...
Please, note that connecting with Channel Managers through TravelgateX requires a previous step in which the Buyer configures the setup of the hotels, rate plans, and rooms contracted with the hoteliers in the [Inventory extranet](https://inventory.xmltravelgate.com/Account/Login?ReturnUrl=%2f). This configuration allows the Channel Manager to map all this information and push availability and prices for the Buyer.
...
For the Channel-X API, it is essential that you [send your endpoint to TravelgateX](making-requests/endpoint).
...
In the case of the Channel-X API, TravelgateX will send requests to the Buyer's system, and the Buyer will respond with either success or error.
