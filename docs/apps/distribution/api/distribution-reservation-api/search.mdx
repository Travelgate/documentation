...
| ------------------------------------- | ---------- | -------- | ----------------------------------------------------- |
| access		| 1  |	node  | Access information: <ul><li>It will be validated to grant access to the API and to the operation.</li><li>If there is any problem then an error will be returned.</li></ul>
| access/accessToken 		| 1  |	string  | Key to access to the API.
...
| hotels            		|0..N|list(string)| Hotels filter:<ul><li>If we just want the reservations of specific hotels then we can set this list with the codes of those hotels.</li><li>If not given or if it is empty then it will not be applied the filter by hotel code.</li></ul>
...
| includeCancellations      | 0..1| boolean  | Cancellations inclusion filter:<ul><li>If set to **_true_** then cancelled reservations that match the filters will be included in the response.</li><li>If not set to **_true_** or not given then only not cancelled bookings will be returned.</li></ul>
...
| reservationWarnings      	| 0,1|list string| If returned then it contains a list of TG locators of reservations that have had problems registering on the main system and that have been stored on an auxiliary system. This information is independent of the searching dates (will be given for any dates range).
| cancellationWarnings      | 0..1|      | If returned then it contains a list of TG locators of cancellations that have had problems registering on the main system and that have been stored on an auxiliary system. This information is independent of the searching dates (will be given for any dates range).
