...
Reservation Read allows to retrieve a specific booking with all its details using the locators, booking codes. The returned fields for each booking in the list include: `Locators`, `Hotel`, `TransactionStatus`, `Holder`, `Price` etc.
...
| --- | --- | --- |
| **ComunicationStatus** | The technical health of the connection between Travelgate and the Seller. | - OK: The communication with the Seller was successful.&nbsp;<br />  - KO: Connection established but failed.&nbsp;<br />  - OFFLINE: Seller system unreachable. |
