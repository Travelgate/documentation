...
* **Code:**  (Optional) Provider/Channel Manager Hotel Code. Only required if channel manager works with their own hotel codes, otherwise leave this field empty. It needs to be unique per `Context`.
* **Context:** (Optional) Provider/Channel Manager that has the hotel. A hotel will only have a specific Context if `Code` has been specified.
...
Bear in mind that a specific `Context` cannot have duplicate hotel codes.
