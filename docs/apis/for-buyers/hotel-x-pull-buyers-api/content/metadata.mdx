...
sidebar_position: 8
---
import HotelXMetadataQueryInput from "../../../../../src/graphql/generated-docs/HotelXMetadataQueryInput.mdx";
import MetadataConnection from "../../../../../src/graphql/generated-docs/MetadataConnection.mdx";
...
* `allowsCurrency`
* `release` (Minimum days required in between booking date and check-in date, if the value is zero then there is no limitation. If the value is -1, Buyers are allowed to make reservations for the day before the check-in. Useful in edge cases where the supplier's timezone is ahead.)
* `maxRelease` (Maximum days required in between booking date and check-in date)
...
:::tip
For a practical overview of how to use Metadata in your integration — including common checks before sending your first requests — visit our [Metadata Knowledge Base article](/kb/connectivity-products/for-buyers/hotel-x/content/metadata). 🚀
:::
