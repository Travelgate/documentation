...
Furthermore, there is an option to load default plugins into our database. Currently, the exclusive method for doing so involves reaching out to our dedicated Customer Care team opening a [ticket](https://app.travelgatex.com/tickets). These default plugins will automatically execute in all the Queries and Mutations when no filters are specified.
...
2. **Wait for Processing:**
   ...
   ```
   [Buyer Context]_[Supplier Context]_hotel_map_.csv
   ```

   After our system processes it, it will look like this:
   ```
   [Buyer Context]_[Supplier Context]_hotel_map_processed.csv
   ```
