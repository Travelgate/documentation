...
* **`parameters`**:
	...
	* **`key`**: "optionsPerKey"
	* **`value`**: Limits the number of options per aggregation group, this being defined by the previous primakyKey parameter. Value: Integer.

:::caution `primaryKey` is a mandatory parameter in order to activate the plugin. `optionsPerKey` is optional and complementary of the first.
:::
