...
| @nonRefundable | 1 |  Boolean | Indicates if it is refundable or not. When `nonRefundable = true` and no `CancelPenalty` is returned, it always means a 100% penalty (of the booking value) from the moment the booking is created, so it is not necessary to return the corresponding `CancelPenalty` range. Even if a non-refundable policy returns penalties, the applied penalty is always 100% of the booking value. |
...
| CancelPenalty/CalculatedDeadline | 1 | Boolean | Specifies if the deadline has been modified by Travelgate by adding 14 hours (Timezone Unknown). |
...
| @binding 				                | 1 		    | Boolean 	    | If binding is set as true, then the client can’t sell the product for a lower price that the one set by the supplier. If set as false, the client can sell the product for a lower price. |
...
| CancelPenalties/CancelPenalty | 0..1|  |  |
| CancelPenalty/HoursBefore | 1 | String | Number of hours prior to arrival day in which this Cancellation policy applies. |
| CancelPenalty/Penalty | 1..n |  | This value represents the penalty amount that will be applied. Normally, it is the amount our client must pay without commission (net). However, if the supplier does not return the net amount during the booking and cancellation flow, and only provides the price with commission included (gross), then the penalty will also be calculated as gross. Some suppliers do not specify whether they work with net or gross prices. In those cases, Travelgate tries to detect it and applies the value that matches the one returned in the cancellation policy. The main goal is to keep consistency with the type of prices the supplier provides throughout the whole booking and cancellation process.|
| @type 				                | 1 		    | String 	    | <details>     <summary>Types of penalties</summary>     <div>         <div>          <table>  				 <thead>  					 <tr>  						 <th>  							 <strong>Penalty</strong>  						 </th>  						 <th>  							 <strong>Description</strong>  						 </th>  					 </tr>  				 </thead>  				 <tbody>  					 <tr>  						 <td>Noches</td>  						 <td>Indicates the number of nights to be penalized.</td>  					 </tr>  					 <tr>  						 <td>Porcentaje</td>  						 <td>Indicates the percentage to pay based on the option price.</td>  					 </tr>           <tr>  						 <td>Importe</td>  						 <td> Indicates the exact amount payable.</td>  					 </tr>				 </tbody>  			</table>         </div>     </div> </details> |
| @currency 				            | 1 		    | String 	    | Currency code (Our system uses a standard ISO - 3 for all suppliers).						                                                        |
| CancelPenalty/CalculatedDeadline | 1 | Boolean | Specifies if the deadline has been modified by Travelgate by adding 14 hours (Timezone Unknown). |
| CancelPenalty/Deadline | 1 | String | Date on UTC Standard TimeZone in which this Cancellation policy applies (ISO 8601 e.g: 2016-07-01T05:00:00Z). | 
