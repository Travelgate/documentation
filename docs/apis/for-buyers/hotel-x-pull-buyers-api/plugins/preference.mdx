...
This is particularly useful when using **HotelX with FastX codes**, where multiple suppliers may offer different prices for the same hotel, board, or room type. With the Preference Plugin, you can **set preference rules** that influence how results are ranked and selected. These rules are managed through the Preference App, which is already pre-installed and ready to use for you in the "My Apps" section of the [Travelgate Store](/kb/platform/app-features/app-store/discover-apps). 
