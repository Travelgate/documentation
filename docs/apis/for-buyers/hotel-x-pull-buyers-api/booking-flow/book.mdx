...
:::warning Important: Test Bookings
...
However, if you prefer testing with real suppliers, please note that any bookings made using test access but involving live suppliers are your responsibility. This means that the Buyer must handle any costs directly with the supplier if bookings are not canceled. Even if an access is labeled as "live" or "production" but uses test credentials internally, you're still responsible for any bookings made. Please keep this in mind when using our API services.
