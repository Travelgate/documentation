...
:::note
This operation simulates what would happen if you execute the `AmendBookingPaxes` operation, without actually committing the changes.
:::

## **Mock**

### **Step 1: Define your AmendBookingPaxes Simulate responses (Mocks)**
...
File location: "**test\MockServer\Tests\AmendBookingPaxes\Two_Adults_Two_Days.txt**".

![Mock Example](/img/docs/connectors-pull-developers-api/MockTwoAdultsAmendPaxes.png)
...
![Response example](/img/docs/connectors-pull-developers-api/TemplateStructureModelsAmendBookingPaxes.png)

**Example of a AmendPaxesSimulateRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Paxes.Models.Request;

public class AmendPaxesSimulateRequest
{
    public List<Occupancy> NewPaxes { get; set; }
...
public class Pax
{
    ...
    public string PersonalTitleType { get; init; }

    public string DateOfBirth { get; set; }
}
```
File location: "**ConnectorsIntegration\Amends\Paxes\Models\Request\AmendPaxesRequest.cs**".

**Example of a AmendPaxesResponse model**:

```csharp
namespace ConnectorsIntegration.Amends.Paxes.Models.Response;

public class AmendPaxesResponse
{
...
```
File location: "**ConnectorsIntegration\Amends\Paxes\Models\Response\AmendPaxesResponse.cs**".
...
File location: "**ConnectorsIntegration\Amends\Paxes\AmendmentsPaxesExtensions.cs**".

If the Seller works with **JSON** format, we can specify the integration to work with **JSON** with the following:

```csharp
internal static class AmendmentsPaxesSimulateExtensions
{
    public static void AddAmendBookingPaxesSimulateServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendPaxesSimulateRequest, AmendPaxesResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingPaxesSimulateOperation<AmendBookingPaxesSimulateOperation, AmendPaxesSimulateRequest, AmendPaxesResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingPaxesSimulateOperation validators**
...
```csharp
internal partial class AmendBookingPaxesSimulateOperation :
    IAmendBookingPaxesSimulateOperation<AmendPaxesSimulateRequest, AmendPaxesResponse, AccessModel>
{
    // Implement any services needed just like in SearchOperation.cs
}
```

File location: "**ConnectorsIntegration\Amends\Paxes\Operations\AmendBookingPaxesOperation.cs**".

![AmendBookingPaxesOperation](/img/docs/connectors-pull-developers-api/AmendBookingPaxesOperationSimulate.png)
...
public bool TryValidateModelRequest(
    AmendBookingPaxesSimulateConnectorRequest connectorsRequest, 
    AmendBookingPaxesSimulateParameters<AccessModel> connectorParameters,
...
public bool TryValidateSupplierResponses(
    AmendBookingPaxesSimulateParameters<AccessModel> connectorParameters,
    IEnumerable<SupplierResponseWrapper<AmendPaxesResponse>> supplierResponses,
    out IEnumerable<AdviseMessage> adviseMessages)
{
    var supplierResponseWrappers = supplierResponses as SupplierResponseWrapper<AmendPaxesResponse>[] ?? supplierResponses.ToArray();
...
- Object of the requests from the models previously created (AmendPaxesSimulateRequest).
- The request that the Buyer sends (connectorsRequest).
- Parameters (connectorParameters) which will have some helpers.

File location: "**ConnectorsIntegration\Amends\Paxes\Operations\AmendBookingPaxesOperation.BuildRequest.cs**"

![Build Request AmendPaxes](/img/docs/connectors-pull-developers-api/BuildRequestAmendPaxesSimulate.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingPaxesSimulate;
using ConnectorsIntegration.Amends.Paxes.Models.Request;

namespace ConnectorsIntegration.Amends.Paxes.Operations;

internal partial class AmendBookingPaxesSimulateOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendPaxesSimulateRequest>> BuildRequests(
        AmendBookingPaxesSimulateConnectorRequest connectorsRequest,
        AmendBookingPaxesSimulateParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingPaxesSimulateRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingPaxesSimulateRq.Input.Reference.SupplierLocator;

        AmendPaxesSimulateRequest amendBookingPaxesSimulateConnectorRequest = BuildAmendBookingPaxesSimulateConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendPaxesSimulateRequest> supplierRequest = new(
            amendBookingPaxesSimulateConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendPaxesSimulateRequest BuildAmendBookingPaxesSimulateConnectorRequest(
        string clientReference,
        string sellerReference) 
    {
        //The request towards the Seller system
      
        return new AmendPaxesSimulateRequest()
        {
            ClientReference = clientReference,
            SellerReference = sellerReference,
            NewPaxes = new List<Occupancy>()
            {
                new Occupancy
                {
                    OccupancyId = 1,
...
:::note
The ParseResponse implementation is shared between Simulate and Commit operations, as they return the same response structure (AmendPaxesResponse).
:::

We will be implementing the "ParseResponse" step inside the Simulate operation:

File location: "**ConnectorsIntegration\Amend\Paxes\Operations\AmendBookingPaxesOperation.ParseResponse.cs**"

![Build Response AmendBookingPaxes](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingPaxesSimulate.png)
...
internal partial class AmendBookingPaxesSimulateOperation
{
    public AmendBookingPaxesSimulateConnectorResponse ParseResponses(
        AmendBookingPaxesSimulateConnectorRequest connectorsRequest,
        AmendBookingPaxesSimulateParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendPaxesResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingPaxesSimulateConnectorResponse(ParseSupplierResponse(supplierResponse));
    }

    private AmendRs ParseSupplierResponse(AmendPaxesResponse supplierResponse)
    {
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingPaxesSimulateDevelopment**" into the original repository.
