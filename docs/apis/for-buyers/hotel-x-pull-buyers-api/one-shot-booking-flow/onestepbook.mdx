...
:::warning Important
 Any mismatch between the data sent in this request and the details provided by ChannelX (e.g., promotion codes, rate types, refundability, etc.) may result in errors or unsuccessful bookings.
:::
...
* `surcharges` 
* `supplements`
* `rateRulesExtended`: Make sure to correctly translate the ChannelX values before sending them in the `RateRulesExtended` input to avoid errors or mismatches.

| ChannelX/Code | ChannelX/PromotionCode | ChannelX/RatePlanType | ChannelX/NonRefundable | OneStepQuote/RateRulesType |
|---------------|-------------------------|------------------------|------------------------|----------------------------------------|
| 0             | No promotion            |                        |                        |                                        |
...
* Consider that the `deltaPrice` sets the price tolerance between Quote and Book. For instance, if the Quote stage displays a price of 120€ and the `deltaPrice.amount` is 10, a price up to 130€ will still secure a confirmed booking. Similarly, if `deltaPrice.percent` is 5 (meaning 5%), a price up to 126€ will be accepted.

* In One Step Book, `deltaPrice` validation checks both the `net` and `gross` prices when comparing the Quote and Book amounts.
...
<details>
	...
	<br/>
		...
		<details>
            <summary>What are the minimum and maximum allowed DeltaPrice percentages?</summary>
            <div>
                The minimum percentage is 0, and there is no maximum limit. You can send values above 100%.
            </div>
        </details>
    </div>
</details>
