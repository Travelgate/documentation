...
With this rule it can be set to activate aggregation of sellings on the HotelX module with a specific key or it can be set not to use any kind of aggregation. Aggregation key has a specific format: it has to be a concatenation of different selectors (the order is important) using ',' as separator (with no blank spaces neither other kind of special characters).
