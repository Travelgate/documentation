...
## When to Use Stats, Stats API, Logging, or Logging API

Use these tools together depending on whether you need dashboard monitoring, programmatic aggregated metrics, UI-based log browsing, or programmatic transaction-level access.
...
| Search, extract traces, and download request/response logs programmatically | [Logging API](/docs/apis/for-buyers-and-sellers/monitoring-api/logging-api/logging) | Returns transaction-level log data for automated audit and support workflows |

For a practical guide to the Stats web UI workflow, see [How to Use Stats](/kb/platform/app-features/monitoring-tools/stats/stats-details) and [Traffic Types](/kb/platform/app-features/monitoring-tools/stats/traffic-types).

## Authentication

All Stats API calls require JWT (JSON Web Token) authentication. Follow the steps in the [Admin API Authentication Guide](/docs/apis/for-buyers-and-sellers/admin-api/authentication) to obtain your bearer token and include it in all API requests.
...
* **The "Other" Operation:** The `OTHER` category covers operations outside the standard booking flow. For HotelX, these are equivalent to:
  ...
  * [Booking Read and Booking List](/docs/apis/for-buyers/hotel-x-pull-buyers-api/booking-management/booking-read/) (ReservationRead, ReservationList)
...
* `timeRange` (Absolute date range. In Monitoring APIs, only `timeRange.absolute` is supported. It accepts both ISO 8601 and `"2006-01-02 15:04:05"` formats).
...
If you need to fetch a dynamic list of valid values to populate your Stats filters, use the following equivalence table to find them via the [Admin API](/docs/apis/for-buyers-and-sellers/admin-api/overview):
...
:::info
For Monitoring APIs, use `timeRange.absolute` in real-time queries as well. The `from` and `to` request values accept both ISO 8601 and `"2006-01-02 15:04:05"` formats.
:::
