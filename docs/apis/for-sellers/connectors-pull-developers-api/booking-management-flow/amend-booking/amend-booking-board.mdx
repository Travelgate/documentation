---
title: "AmendBookingBoard"
sidebar_position: 2
---

## **Overview**

The `AmendBookingBoard` operation allows clients to modify the board type (meal plan) of an existing booking. This operation uses the client and supplier locators to identify the reservation and processes the board change request.

The `AmendBookingBoard` operation ensures:
...
<Tabs>
  <TabItem value="AmendBookingBoardRequest" label="C#">
    ```csharp
    var amendBookingBoardRq = new AmendBookingBoardRq
    {
        Input = new AmendBoardInput
        {
            Language = "99",
...
<!-- TABLE BEGIN Connectors.Pull.Hotel.Domain.Contracts.Amendments.AmendBookingBoard.Request.AmendBookingBoardRq -->
| Name | Relation | Type | Description |
|------|:--------:|:----:|-------------|
| **Input** | 1 | [AmendBoardInput](/docs/apis/for-sellers/connectors-pull-developers-api/api-reference/amendboardinput) |  Gets or initializes the input details for amending the board type, including specific information about the booking and the new board type. |
...
The `AmendBookingBoard` operation provides:
- **Amendment Status**:
...
<Tabs>
  <TabItem value="AmendBookingBoardResponse" label="C#">
    ```csharp
