...
This call returns the bookings a Buyer made against the Seller. It is **not** a way for a Seller to fetch the bookings received in our platform: the Seller's own system only receives and answers the request.
:::
...
### Understanding TransactionStatus fields

When managing bookings, it is important to distinguish between the **technical communication, the record availability, and the actual status of a reservation**.

| Field | Purpose | Key Values |
| --- | --- | --- |
| **ComunicationStatus** | The technical health of the connection between Travelgate and the Seller. This legacy XML field may also be referred to in prose as "CommunicationStatus". | - OK: The communication with the Seller was successful.&nbsp;<br />  - KO: Connection established but failed.&nbsp;<br />  - OFFLINE: Seller system unreachable. |
| **RSStatus** | Indicates if the reservation record exists in the Seller’s database. | - EXISTE: Found. &nbsp;<br /> - NO_EXISTE: Not found.&nbsp;<br />  - EXISTECANCELADA: Found but cancelled.&nbsp;<br /> - DESCONOCIDO: Unknown. |
| **ResStatus** | The actual status of the booking itself. | - OK: Confirmed. &nbsp;<br /> - RQ: On Request (Waiting list). &nbsp;<br /> - CN: Cancelled. &nbsp;<br /> - UN: Unknown (Buyer follow-up is required). |
