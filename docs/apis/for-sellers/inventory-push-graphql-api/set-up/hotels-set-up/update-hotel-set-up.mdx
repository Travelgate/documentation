---
sidebar_position: 2
---


import HotelsSetupRs from "../../../../../../src/graphql/generated-docs/HotelsSetupRs.mdx";
import InventoryHotelsSetupUpdateInput from "../../../../../../src/graphql/generated-docs/InventoryHotelsSetupUpdateInput.mdx";

import {updateHotelSetUpMutation, updateHotelSetUpVariables} from "/src/graphql/inventory/set-up/hotels-set-up.mutation";

# Update Hotel

## Mutation Overview

The `updateHotelSetup` mutation allows you to update the configuration of a hotel already added to a specific client-seller relation in Inventory. The returned fields include:

* `id`
* `hotelCode`
* `contextCode`
* `active`
* `sendNotification`
* `emailNotification`
* `emailNotificationCc`
* `emailNotificationSupport`

:::info Extranet and API behavior
In Inventory Extranet, `hotelCode` and `contextCode` become immutable once the hotel is created. If you need different values there, create a new hotel.

In `updateHotelSetup`, `hotelCode` is used to identify the target hotel setup, and `contextCode` can be sent in the update payload for API-based integrations.
:::

### 1. Criteria

When building your mutation, you need to provide the following input fields:

#### Mandatory Input
* `clientCode`
* `supplierCode`
* `hotelCode`

#### Optional Input
* `contextCode`
* `active`
* `sendNotification`
* `emailNotification`
* `emailNotificationCc`
* `emailNotificationSupport`

### 2. Settings

To update an existing hotel setup, specify the necessary fields while ensuring that the `clientCode`, `supplierCode`, and `hotelCode` are correctly associated with the targeted hotel configuration.

### Response Considerations

The mutation returns the updated hotel setup along with success status and possible advise messages.

#### `HotelsSetupRs` (*OBJECT*)
* `hotels` (*InventoryHotelSetup*) - A collection of updated hotels.
    * `id` (*Int*) - Unique identifier.
    * `hotelCode` (*String*) - Associated hotel code.
    * `contextCode` (*String*) - Context code for the hotel.
    * `active` (*Boolean*) - Indicates if the hotel is active.
    * `sendNotification` (*Boolean*) - Indicates if booking notifications should be sent.
    * `emailNotification` (*String*) - Email addresses for notifications.
    * `emailNotificationCc` (*String*) - CC email addresses for notifications.
    * `emailNotificationSupport` (*String*) - Support contact emails.
* `success` (*Boolean*) - Indicates if the operation was successful.
* `adviseMessages` (*AdviseMessage*) - Messages related to the operation.


## Mutation Inputs

<InventoryHotelsSetupUpdateInput />
## Returned Fields

<HotelsSetupRs />


## Examples

#### Update the configuration from a Hotel

<GraphqlSample query={updateHotelSetUpMutation} variables={updateHotelSetUpVariables} />