...
As in previous tools of the extranet, you will need to select the client and the provider. Then, you will be able to search for the hotel using the following filter fields:

* **Hotel ID:** Filter by the hotel's internal Inventory ID.
* **Hotel Name:** Filter by the hotel's name.
* **Hotel Code:** Filter by the provider's own hotel code (if assigned).
* **Date From:** Starting date for the calendar view.

Once you have selected the hotel, choose the rate for which you want to check the information that has been loaded in the system. A filter that allows you to filter per room or derived rate will also appear. These two options, unlike the rate, are optional and if you do not indicate information in them, all product loaded for the rate will be displayed.

![Inventory Calendar and Availability](inventory_calendar2.png)

### 🔍 Enhanced Filter Search

We've integrated a **dynamic search bar** directly into the filter dropdowns within the **Calendar** view to streamline your workflow and save you valuable time.

#### What's New?
The search functionality is now available for both **Rates** and **Rooms** dropdowns, allowing you to quickly find exactly what you need without endless scrolling—especially useful when managing hotels with hundreds of rates and rooms.

#### How to Use It
Simply open the **Rates** or **Rooms** dropdown menu in the Calendar filters and start typing the name or code. The list will **filter in real-time** as you type!

![Inventory Calendar Filter with Search](FilterSearchCalendar.png)

#### Why You'll Love It
* **Instant Access:** No more scrolling through endless lists to find a specific rate or room.
* **Dual-Level Filtering:** The search functionality is available for both **Rates** and **Rooms**.
* **Increased Precision:** Quickly pinpoint the exact data you need to review or update.
...
In order to simulate the availability requests and response for the product loaded in Inventory you will need to go to the `Product` tab and then select the `Availability` option in the Inventory extranet.
...
* **Hotel Code**: The provider's own hotel code (if assigned). This field allows you to identify the hotel using the channel manager's code.
...
Once you have filled all the information you can click on the `Search` button and all options and prices will appear if there is any rate that fulfills all conditions.
