...
You may be using the HotelX Pull Buyers API in single mode (request for a specific supplier) or in multi mode (request for different suppliers at the same time). Depending on your mode, you might want to use this plugin in different ways:
