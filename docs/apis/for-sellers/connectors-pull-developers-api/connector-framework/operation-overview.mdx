...
```csharp
public partial class SearchOperation : ISearchOperation<SearchRequest, SearchResponse, AccessModel>
{
...
{
    services.AddSearchOperation<SearchOperation, SearchRequest, SearchResponse, AccessModel>(
...
In our Content you are not expected to return the mapped element. You need to add all the mapped elements to our global variable `_accumulator`. This allows us to validate and process the content efficiently in small batches, before storing it in our database.
