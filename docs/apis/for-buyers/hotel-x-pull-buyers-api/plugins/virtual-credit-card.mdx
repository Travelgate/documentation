...
Once you've uploaded your genvcc file, you can use the plugin adding the following field to the `settings` node of your request. Note that genvcc plugin can just be executed in `book` `mutation`. In order to successfully execute the genvcc plugin, you will need to create the request with the following variables.
