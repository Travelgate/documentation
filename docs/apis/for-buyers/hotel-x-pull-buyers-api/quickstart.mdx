...
[`OptionRefID`](/kb/our-products/are-you-a-buyer/getting-started-with-hotel-x-buyers-api/understanding-the-id-optionRefId-of-an-option) is a unique identifier assigned to each option. **It must not be modified** and **must not be used or interpreted externally**, as it is an internal TGX system identifier.
