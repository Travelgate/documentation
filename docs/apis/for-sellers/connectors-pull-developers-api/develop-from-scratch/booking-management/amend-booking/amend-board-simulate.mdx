...
### **Step 1: Define your AmendBookingBoard Simulate responses (Mocks)**
...
File location: "**test\MockServer\Tests\AmendBookingBoard\Two_Adults_Two_Days.txt**".

![Mock Example](/img/docs/connectors-pull-developers-api/MockTwoAdultsAmendBoard.png)
...
![Response example](/img/docs/connectors-pull-developers-api/TemplateStructureModelsAmendBookingBoard.png)

**Example of a AmendBoardSimulateRequest model**:

```csharp
namespace ConnectorsIntegration.Amends.Board.Models.Request;

public class AmendBoardSimulateRequest
{
...
```
File location: "**ConnectorsIntegration\Amends\Boards\Models\Request\AmendBoardRequest.cs**".

**Example of a AmendBoardResponse model**:

```csharp
namespace ConnectorsIntegration.Amends.Board.Models.Response;

public class AmendBoardResponse
{
...
```
File location: "**ConnectorsIntegration\Amends\Board\Models\Response\AmendBoardResponse.cs**".
...
File location: "**ConnectorsIntegration\Amends\Board\AmendmentsBoardExtensions.cs**".

If the Seller works with **JSON** format, we can specify the integration to work with **JSON** with the following:

```csharp
internal static class AmendmentsBoardSimulateExtensions
{
    public static void AddAmendBookingBoardSimulateServices(this IServiceCollection services,
        IConfiguration configuration)
    {
        //A JsonSerializer service is added along with the request and response model
        services.AddJsonSerializer<AmendBoardSimulateRequest, AmendBoardResponse>(ConfigureJSONOptions);
        //The operation is added, indicating what models should be used during the development of the operation
        services.AddAmendBookingBoardSimulateOperation<AmendBookingBoardSimulateOperation, AmendBoardSimulateRequest, AmendBoardResponse, AccessModel>(TgxPlatform.Name, configuration);
    }
...
### **Step 2: AmendBookingBoardSimulateOperation validators**
...
```csharp
internal partial class AmendBookingBoardSimulateOperation :
    IAmendBookingBoardSimulateOperation<AmendBoardSimulateRequest, AmendBoardResponse, AccessModel>
{
    // Implement any services needed just like in SearchOperation.cs
}
```

File location: "**ConnectorsIntegration\Amends\Board\Operations\AmendBookingBoardOperation.cs**".

![AmendBookingBoardOperation](/img/docs/connectors-pull-developers-api/AmendBookingBoardSimulateOperation.png)
...
public bool TryValidateModelRequest(
    AmendBookingBoardSimulateConnectorRequest connectorsRequest, 
    AmendBookingBoardSimulateParameters<AccessModel> connectorParameters,
...
public bool TryValidateSupplierResponses(
    AmendBookingBoardSimulateParameters<AccessModel> connectorParameters,
    IEnumerable<SupplierResponseWrapper<AmendBoardResponse>> supplierResponses,
    out IEnumerable<AdviseMessage> adviseMessages)
{
    var supplierResponseWrappers = supplierResponses as SupplierResponseWrapper<AmendBoardResponse>[] ?? supplierResponses.ToArray();
...
- Object of the requests from the models previously created (AmendBoardSimulateRequest).
- The request that the Buyer sends (connectorsRequest).
- Parameters (connectorParameters) which will have some helpers.

File location: "**ConnectorsIntegration\Amends\Board\Operations\AmendBookingBoardOperation.BuildRequest.cs**"

![Build Request AmendBoard](/img/docs/connectors-pull-developers-api/BuildRequestAmendBoardSimulate.png)

**Example of Build Request:**

```csharp
using Connectors.Core.Application.Connection;
using Connectors.Pull.Hotel.Application.Operations.Amendments.AmendBookingBoardSimulate;
using ConnectorsIntegration.Amends.Board.Models.Request;

namespace ConnectorsIntegration.Amends.Board.Operations;

internal partial class AmendBookingBoardSimulateOperation
{
    public IEnumerable<SupplierRequestWrapper<AmendBoardSimulateRequest>> BuildRequests(
        AmendBookingBoardSimulateConnectorRequest connectorsRequest,
        AmendBookingBoardSimulateParameters<AccessModel> connectorParameters)
    {
        var clientReference = connectorsRequest.AmendBookingBoardSimulateRq.Input.Reference.ClientLocator;
        var supplierReference = connectorsRequest.AmendBookingBoardSimulateRq.Input.Reference.SupplierLocator;

        AmendBoardSimulateRequest amendBookingBoardSimulateConnectorRequest = BuildAmendBookingBoardSimulateConnectorRequest(
            clientReference,
            supplierReference);
        ...
        SupplierRequestWrapper<AmendBoardSimulateRequest> supplierRequest = new(
            amendBookingBoardSimulateConnectorRequest,
            new Uri(genericUrl),
            HttpMethod.Post);
    ...
    private static AmendBoardSimulateRequest BuildAmendBookingBoardSimulateConnectorRequest(
        string clientReference,
        string sellerReference) 
    {
        //The request towards the Seller system
      
        return new AmendBoardSimulateRequest()
        {
...
:::note
The ParseResponse implementation is shared between Simulate and Commit operations, as they return the same response structure (AmendBoardResponse).
:::

We will be implementing the "ParseResponse" step inside the Simulate operation:

File location: "**ConnectorsIntegration\Amend\Boards\Operations\AmendBookingBoardOperation.ParseResponse.cs**"

![Build Response AmendBookingBoard](/img/docs/connectors-pull-developers-api/ParseResponseAmendBookingBoardsSimulate.png)
...
internal partial class AmendBookingBoardSimulateOperation
{
    public AmendBookingBoardSimulateConnectorResponse ParseResponses(
        AmendBookingBoardSimulateConnectorRequest connectorsRequest,
        AmendBookingBoardSimulateParameters<AccessModel> connectorParameters,
        IEnumerable<SupplierResponseWrapper<AmendBoardResponse>> supplierResponses,
        CancellationToken cancellationToken)
    {
        ...
        return new AmendBookingBoardSimulateConnectorResponse(ParseSupplierResponse(supplierResponse));
    }

    private AmendRs ParseSupplierResponse(AmendBoardResponse supplierResponse)
    {
...
### **Step 1: Create Pull Request**
    ...
    - Commit your changes and push them to a new branch called "**AmendBookingBoardSimulateDevelopment**" into the original repository.
