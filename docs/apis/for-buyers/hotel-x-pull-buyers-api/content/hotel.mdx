...
import {hotelsQuery, hotelsByCountryVariables, hotelsByDestinationCodeVariables, hotelsByHotelCodeVariables} from "../../../../../src/graphql/hotel-x/hotels.query";
...
When creating your hotels query, you have four different inputs to fill based on your specific needs:
...
The Hotel-X API provides paginated responses to handle cases where the number of hotels from the selected supplier exceeds 5000. The default response page contains a maximum of 5000 hotels, but you can specify the number of objects returned per page using the `maxSize` field in the criteria variable explained before to retrieve between 1 and 5000 hotels.
...
First hotels query you won't indicate any `token`input:
...
#### Search for specific hotel code, country or destination

When utilizing the Hotels List Query, for more personalized results, filter specific hotels, countries or destinations based on your interests. For example, to focus solely on the country Greece inventory of a Seller, add the `cpuntries` field with "GR" in your criteria. This will ensure you receive hotels directly related to that country, providing you with highly targeted and relevant information.

<GraphqlSample query={hotelsQuery} variables={hotelsByCountryVariables} />

You would need to apply the same approach for search for specific hotels (`hotelCodes`) and destinations (`destinationCodes`). For example, if we want to obtain only hotels from "Santorini", we can use the destination code for that zone. In our case, our test suppliers as code "101402" as Santorini, so we need to specify this code in the `destinationCodes` field:

<GraphqlSample query={hotelsQuery} variables={hotelsByDestinationCodeVariables} />

And if we want a list of specific hotel codes:
