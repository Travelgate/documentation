...
Remember that when making the [`search`](../apis/for-buyers/hotel-x-pull-buyers-api/booking-flow/search.mdx) query, in the criteria, you will need to add the `destinations` field (instead of `hotels`). There is where you will need to add the destination codes. 

To find out which destinations codes an access has, you will need to make the [`destination`](../apis/for-buyers/hotel-x-pull-buyers-api/content/destination.md) content call.
