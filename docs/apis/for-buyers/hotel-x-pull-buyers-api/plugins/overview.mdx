...
## Related plugin pages

- [Mapping](./mapping.mdx)
- [Currency Converter](./currency-converter.mdx)
- [Cheapest Price](./cheapest-price.mdx)
...
Where "unique code" corresponds to the HotelX unique code in our system.
...
| File status | Example |
| --- | --- |
| Uploaded (pending processing) | `BuyerContext_SupplierContext_hotel_map_.csv` |
| Processed by Travelgate | `BuyerContext_SupplierContext_hotel_map_processed.csv` |
...
:::tip
Before testing in production, confirm for each plugin whether it needs only SFTP data, only request parameters, or both at the same time.
:::
