...
- **Initiates the call:** Travelgate.
- **Processes and returns the data:** the Seller (its connection configuration template).
- **Receives the result:** Travelgate, which builds the activation form that Buyers complete to connect to the Seller's system.
:::
...
<ConfigurationRS xmlns:xsi = "http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd = "http://www.w3.org/2001/XMLSchema">
   ...
   <Parameters>
      <Parameter>
         ...
         <Description>Supplier Endpoint</Description>
         <Type>uri</Type>
         <DefaultValue>http://travelgate.com/</DefaultValue>
      </Parameter>
      <Parameter>
      ...
      <Parameter>
         ...
         <Description>Affiliation of the user, affiliation of the XML account created. Two digit code. This code refers to which brand it belongs to</Description>
         <Type>string</Type>
         <Key>Afiliacion</Key>
      ...
      <Parameter>
       ...
       <Parameter>
          ...
          <DefaultValue>https://xtghubstatic.blob.core.windows.net/int-static-hotel/tgxStaticRoomTypes.csv</DefaultValue>
      </Parameter>
   </Parameters>
</ConfigurationRS>  
```
...
| Parameter/ProviderTagName	    | 1        	    | String        | Name of the parameter in the activation form, as you use our PULL Seller API, it is advisable to put similar names used on it in a human readability format. For example: _Availability Url_ or _Valuation Url_. If it is an extra parameter, you should put a name similar to the key used, and preferably with a human readability format too. 				| 
...
| Parameter/Type	            |1          	| Enum          |  The parameter value data type. Possible types are:  `eString`, `eBoolean`, `eUri`. 				| 
| Parameter/Pattern	            |0..1          	| String        | Regular expression to validate the parameter value. If the parameter type is `eUri` or `eBoolean` the pattern is mandatory, and has a specific regular expression. You can find more information about patterns [here](/docs/apis/for-sellers/deprecated/hotel-pull-sellers-api/content/configuration#patterns-for-euri-and-eboolean). If the type is `eString` or you add the tag `PossibleValues`, the pattern is not mandatory.| 
...
| Parameter/Description	        |0..1          	|String         |  Value of the tooltip that explains the parameter. Avoid use an only word, avoid use the same value than `ProviderTagName`, and if it is an additional parameter, avoid use the same value than `Key`. Please, fill this information to help the Buyer to connect you.| 
...
All the patterns must work with Golang and ECMAScript (javaScript). You can validate your regular expressions by this two languages at <a href="https://regex101.com/">regex101</a>
